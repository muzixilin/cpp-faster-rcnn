...
namespace caffe {
	...
	void RPNLayer<Dtype>::proposal_local_anchor(){
		...
		int nRequiredMem = length*sizeof(int)+2 * step*sizeof(Dtype);
		int *map_m = 0;	Dtype *shift_x = 0, *shift_y = 0;
		if (nRequiredMem <= buffer_size_)
		{
			map_m = (int*)(pBuffer_);
			shift_x = (Dtype*)(pBuffer_ + sizeof(int)*length);
			shift_y = shift_x + step;
		}
		else
		{
			map_m = new int[length];			
			shift_x = new Dtype[step];
			shift_y = new Dtype[step];
		...
		if (nRequiredMem > buffer_size_)
		{
			delete[] shift_y; shift_y = 0;
			delete[] shift_x; shift_x = 0;
			delete[] map_m; map_m = 0;
		}
	...
	void RPNLayer<Dtype>::Forward_cpu(
		...
		//SaveBlob2txt("delta_box.txt", *m_box_);
		...
		//SaveBlob2txt("score.txt",*m_score_);
		//SaveBlob2txt2("original_boxscore.txt", *m_box_, *m_score_);
		//RestoreBlobFromtxt("original_boxscore161.txt", *m_box_, *m_score_);
		...
		//std::cout << "h,w,s" << src_height_ << " " << src_width_ << " " << src_scale_<< std::endl;
		...
		/*std::ofstream outfile("pre-nms.txt", std::ios::out);
		for (int i = 0; i < aboxes.size(); i++)
		{
			outfile << aboxes[i].x1 << " " << aboxes[i].y1 << " ";
			outfile << aboxes[i].x2 << " " << aboxes[i].y2 << " ";
			outfile << aboxes[i].score << std::endl;
		}
		outfile.close();*/
		...
		/*std::ofstream outfileb("after-nms.txt", std::ios::out);
		for (int i = 0; i < aboxes.size(); i++)
		{
			outfileb << aboxes[i].x1 << " " << aboxes[i].y1 << " ";
			outfileb << aboxes[i].x2 << " " << aboxes[i].y2 << " ";
			outfileb << aboxes[i].score << std::endl;
		}
		outfileb.close();*/
		/*std::ifstream outfileb("afternms_box_score161.txt", std::ios::in);
		for (int i = 0; i < aboxes.size(); i++)
		{
			...
			outfileb >> y2;
			outfileb >> score;
			printf("before: %f, %f, %f, %f, %f  ", x1, y1, x2, y2, score);
			printf("aboxex: %f, %f, %f, %f, %f  ", aboxes[i].x1, aboxes[i].y1, aboxes[i].x2, aboxes[i].y2, aboxes[i].score);
			aboxes[i].x1 = x1;
			aboxes[i].y1 = y1;
			aboxes[i].x2 = x2;
			aboxes[i].y2 = y2;
			aboxes[i].score = score;
	...
	template <typename Dtype>
	void RPNLayer<Dtype>::SaveBlob2txt(char* szSaveFileName, Blob<Dtype>& blob)
	{
		if (!szSaveFileName){
			return;		
		}
		std::ofstream outfile;
		outfile.open(szSaveFileName, std::ios::out);
	...
	template <typename Dtype>
	void RPNLayer<Dtype>::SaveBlob2txt2(char* szSaveFileName, Blob<Dtype>& boxes, Blob<Dtype>& scores)
	{
		if (!szSaveFileName){
			return;
		}
	...
	template <typename Dtype>
	void RPNLayer<Dtype>::RestoreBlobFromtxt(char* szSaveFileName, Blob<Dtype>& boxes, Blob<Dtype>& scores)
	{
		if (!szSaveFileName){
			return;
		}
			...
			for (int p = 0; p < h; p++)
			{
				for (int q = 0; q < w; q++)
				{
					for (int j = 0; j < c / 2; j++)
					{
						for (int l = 0; l < 4; l++)
						{
							printf("before=%f", boxes.data_at(i, 4 * j + l, p, q));
							infile >> tt;
							boxes.data_set(tt, i, 4 * j + l, p, q);
							printf("after=%f\n", boxes.data_at(i, 4 * j + l, p, q));
						}
						infile >> tt;
						scores.data_set(tt, i, j + c / 2, p, q);
					}
				}
